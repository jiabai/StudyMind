...
- 身份边界测试递归扫描 Server 源码、测试、脚本、Prisma、环境模板和 Rust 账号入口；错误包含文件和行号。
- 扫描器先用纯内存禁用词 fixture 证明 RED，随后清除测试/Rust 负向样例中的字面旧身份并转绿。
...
- 全量 Cargo 的失败集中在既有 annotation/settings/progress/worker watchdog 测试。本次修改仅将三个 legacy ticket 负向 fixture 改为编译期拼接；直接覆盖的 auth callback 5/5 和 account 5/5 均通过，未扩大范围修复无关 Rust 问题。
