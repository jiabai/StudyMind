...
export class UserAuthService {
  ...
  constructor(private readonly options: { store: UserStore; sendOtp: (email: string, code: string) => Promise<void>; otpHmacKey: string; now?: () => Date }) {
  ...
  async startEmailLogin(input: { email: string; state: string; ip: string }) {
    ...
    const issued = await this.options.store.issueEmailOtp({ purpose: "desktop_login", email, state: input.state, ip: normalizeAuthIp(input.ip), codeHash: hashEmailOtp({ key: this.options.otpHmacKey, purpose: "desktop_login", email, state: input.state, code }), expiresAt: new Date(now.getTime() + 600_000), createdAt: now });
  ...
  async verifyEmailCode(input: { email: string; state: string; code: string }) {
    ...
    const result = await this.options.store.verifyUserOtpAndCreateWebSession({ email, state: input.state, codeHash: hashEmailOtp({ key: this.options.otpHmacKey, purpose: "desktop_login", email, state: input.state, code: input.code }), sessionTokenHash: sha256(sessionToken), csrfTokenHash: sha256(csrfToken), now, sessionExpiresAt: new Date(now.getTime() + TTL) });
