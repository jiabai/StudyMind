...
export class AdminAuthService {
  ...
  async startEmailLogin(input: { state: string; ip: string }) {
    const email = this.adminEmail; validateState(input.state);
  ...
  async verifyEmailCode(input: { state: string; code: string }) {
    const email = this.adminEmail; validateState(input.state);
