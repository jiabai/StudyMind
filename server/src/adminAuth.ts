...
export class AdminAuthService {
  ...
  constructor(private readonly options: { store: AdminStore; sendOtp: (email: string, code: string) => Promise<void>; adminEmail?: string; now?: () => Date }) {
  ...
  async startEmailLogin(input: { email: string; state: string; ip: string }) {
    const email = normalizeEmail(input.email); validateState(input.state); if (!constantTimeEqual(email, this.adminEmail)) throw new Error("ADMIN_ONLY");
    const now = this.now(); const code = otpCode();
    const issued = await this.options.store.issueEmailOtp({ purpose: "admin_login", email, state: input.state, ip: input.ip, codeHash: sha256(code), expiresAt: new Date(now.getTime() + 600_000), createdAt: now });
  ...
  async verifyEmailCode(input: { email: string; state: string; code: string }) {
    ...
    const result = await this.options.store.verifyAdminOtpAndCreateSession({ email, state: input.state, codeHash: sha256(input.code), sessionTokenHash: sha256(sessionToken), csrfTokenHash: sha256(csrfToken), now, sessionExpiresAt: new Date(now.getTime() + TTL) });
