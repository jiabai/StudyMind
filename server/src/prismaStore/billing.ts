...
function payload(input: Input): string { return JSON.stringify({ outTradeNo: input.outTradeNo, transactionId: input.transactionId, paidAt: input.paidAt.toISOString(), passDays: input.passDays }); }
...
export async function settlePaidOrder(prisma: PrismaClient, input: Input): ReturnType<Store["settlePaidOrder"]> {
  const result = await withConflictRetry(() => prisma.$transaction(async (tx) => {
    ...
    if (event) { if (event.outTradeNo !== input.outTradeNo) return { status: "webhook_order_mismatch" } as const; if (event.payload !== payload(input)) return { status: "webhook_payload_conflict" } as const; const entitlement = await tx.entitlement.findUnique({ where: { userId: order.userId } }); return entitlement && order.status === "paid" && order.transactionId === input.transactionId ? { status: "settled", entitlement: entitlement as EntitlementRecord } as const : { status: "order_state_conflict" } as const; }
    ...
    if (order.status === "paid") { if (order.transactionId !== input.transactionId) return { status: "transaction_mismatch" } as const; await tx.webhookEvent.create({ data: { id: randomUUID(), provider: input.provider, eventId: input.eventId, outTradeNo: input.outTradeNo, payload: payload(input), createdAt: input.now } }); const entitlement = await tx.entitlement.findUnique({ where: { userId: order.userId } }); return entitlement ? { status: "settled", entitlement: entitlement as EntitlementRecord } as const : { status: "order_state_conflict" } as const; }
