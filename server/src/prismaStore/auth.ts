...
import { isUnique, RateLimitExceededError, rateLimitReservations, reserveRateLimit, withConflictRetry } from "./concurrency.js";
  ...
  const result = await withConflictRetry(async () => {
    try {
      return await prisma.$transaction(async (tx) => {
        for (const reservation of rateLimitReservations(input)) await reserveRateLimit(tx, reservation, input.createdAt);
    ...
    } catch (error) { if (error instanceof RateLimitExceededError) return { status: "rate_limited", retryAt: error.retryAt } as const; throw error; }
  });
...
async function verifyOtp(tx: Prisma.TransactionClient, purpose: "desktop_login" | "admin_login", input: { email: string; state: string; codeHash: string; now: Date }): Promise<boolean> {
  ...
  const changed = await tx.emailOtp.updateMany({ where: { id: otp.id, consumedAt: null, attempts: { lt: 5 }, expiresAt: { gt: input.now } }, data: matches ? { attempts: { increment: 1 }, consumedAt: input.now } : { attempts: { increment: 1 } } });
  return matches && changed.count === 1;
}
...
export async function verifyDesktopOtpAndCreateTicket(prisma: PrismaClient, input: Parameters<Store["verifyDesktopOtpAndCreateTicket"]>[0]): ReturnType<Store["verifyDesktopOtpAndCreateTicket"]> {
  const result = await withConflictRetry(() => prisma.$transaction(async (tx) => {
    ...
    if (!await verifyOtp(tx, "desktop_login", input)) return { status: "invalid" } as const;
...
export async function verifyAdminOtpAndCreateSession(prisma: PrismaClient, input: Parameters<Store["verifyAdminOtpAndCreateSession"]>[0]): ReturnType<Store["verifyAdminOtpAndCreateSession"]> {
  const result = await withConflictRetry(() => prisma.$transaction(async (tx) => {
    ...
    if (!await verifyOtp(tx, "admin_login", input)) return { status: "invalid" } as const;
