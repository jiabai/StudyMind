...
import { AuthService, userSessionMaxAgeSeconds } from "../auth.js";
...
export function registerDesktopAuthRoutes(app: FastifyInstance, dependencies: Dependencies): void {
  ...
  app.post("/auth/email/start", async (request, reply) => {
    ...
    try { await auth.startEmailLogin({ ...parsed.data, ip: request.ip }); return { ok: true }; } catch (error) { return sendAuthError(reply, error); }
  });
...
function sendAuthError(reply: { code(status: number): { send(value: unknown): unknown } }, error: unknown) {
