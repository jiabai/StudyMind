...
export function registerUserAuthRoutes(app: FastifyInstance, d: Dependencies) {
  ...
  app.post("/user/auth/logout", async (request, reply) => { const cookies = parseCookies(request.headers.cookie); const token = cookies.get("studymind_user_session") ?? null; const session = await d.auth.authenticate(token); if (!session || !token) return reply.code(401).send({ error: "AUTH_REQUIRED" }); const csrf = firstHeader(request.headers["x-studymind-csrf"]); if (!csrf || csrf !== cookies.get("studymind_user_csrf") || !d.auth.validateCsrf(session, csrf)) return reply.code(403).send({ error: "CSRF_INVALID" }); await d.store.revokeUserSession(sha256(token), now()); clearCookie(reply, "studymind_user_session", true, secure); clearCookie(reply, "studymind_user_csrf", false, secure); return { ok: true, redirect_url: "/login" }; });
}
function authError(reply: { code(status: number): { send(value: unknown): unknown } }, error: unknown) { if (isServerTemporarilyUnavailable(error)) return reply.code(503).send({ error: "SERVER_TEMPORARILY_UNAVAILABLE" }); const message = publicAuthError(error); return message ? reply.code(400).send({ error: message }) : reply.code(500).send({ error: "INTERNAL_SERVER_ERROR" }); }
